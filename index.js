...
app.use((req, res, next) => {
	if (!req.session.authenticated && !req.path.includes('/api/authenticate'))
...
app.post('/api/projects', async (req, res) => {
	...
	for (let i = 1; i < projectCSV.length; i++) {
		...
		await db.query(
			'INSERT INTO projects(name, github, categories, projectId) VALUES($1 ,$2, $3, $4)',
			[
...
app.put('/api/projects', async (req, res) => {
	...
	if (tableNum * tablesCSV.length * waveNum < projectNum) {
		...
		for (let i = 1; i < projectNum; i++) {
			await db.query(
				'UPDATE projects SET tableName = $1 WHERE projectId = $2;',
				[tablesCSV[t][0], i]
			)
			t++
...
app.get('/api/unscored/judge/:judgeId', async (req, res) => {
	try {
		const { judgeId } = req.params
		const query = await db.query(
			'SELECT DISTINCT ON (projects.projectId) projects.projectId, projects.name, projects.categories, projects.github, projects.tableName, projects.wave, filtered.score FROM projects INNER JOIN (SELECT * FROM scores WHERE scores.judgeId = $1 AND scores.score IS NULL) AS filtered ON projects.projectId=filtered.projectId;',
			[judgeId]
		)
		res.send(query.rows)
	} catch (error) {
...
app.get(
	...
	async (req, res) => {
		try {
			...
			const query = await db.query(
				'SELECT category, score FROM scores WHERE judgeId = $1 AND projectId = $2;',
				[judgeId, projectId]
			)
			res.send(query.rows)
		} catch (error) {
...
app.put(
	...
	async (req, res) => {
		...
		await db.query(
			'UPDATE scores SET score = $1 WHERE judgeId = $2 AND projectId = $3 AND category = $4;',
			[score, judgeId, projectId, category]
		)
...
function getApiMapping(apisJSON, judgeJSON) {
	...
	for (j = 0; j < judgeJSON.length; j += 1) {
		...
		apiMappings[api].judges = apiMappings[api].judges.concat(
			judgeJSON[j].judgeid
		)
	}
	return apiMappings
			...
			for (j = 0; j < categories.length; j += 1) {
				...
				if (currCat.slice(0, 3) === 'GC:') {
					...
					if (hasGC) {
						...
						let apiIndex =
							(apiMappings[currCatKey].index - 1) % apiJudges.length
						if (apiIndex < 0) {
							apiIndex = apiJudges.length - 1
						}
						await db.query(
							'INSERT INTO scores(judgeID, projectID, category) VALUES ($1, $2, $3)',
							[apiJudges[apiIndex], currProj.projectid, currCat]
						)
						continue
					} else {
						hasGC = true
...
app.get('/api/winners', async (req, res) => {
	try {
		const apis = await db.query(
			'SELECT DISTINCT category FROM scores ORDER BY category;'
		)
		const apisJSON = apis.rows
		const winnersJSON = {}
		let currCat, query
		for (let i = 0; i < apisJSON.length; i += 1) {
			...
			query = await db.query(
				'SELECT * FROM scores WHERE category = $1 ORDER BY score DESC;',
				[currCat]
			)
			projects = query.rows
			winnersJSON[currCat] = []
			...
			for (let j = 0; j < projects.length; j += 1) {
				...
				judgeNameQuery = await db.query(
					'SELECT name FROM judges WHERE judgeId = $1',
					[currJudgeID]
				)
				judgeName = judgeNameQuery.rows[0].name

				winnersJSON[currCat] = winnersJSON[currCat].concat([
					{ projectname: name, judgename: judgeName, score: projects[j].score }
				])
			}
		}

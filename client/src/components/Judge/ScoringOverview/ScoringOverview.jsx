...
class ScoringOverview extends Component {
  ...
  async componentDidMount() {
    const resUnscored = await fetch(
      '/api/unscored/judge/' + this.props.location.state.judgeId,
      { credentials: 'include' }
    );
    const resJson1 = await resUnscored.json();
    const resScored = await fetch(
      '/api/scored/judge/' + this.props.location.state.judgeId,
      { credentials: 'include' }
    );
      ...
      <div className="scoring-view">
        <div className="score-button-box">
          <div className="score-button">
            <button
              className={this.state.showUnscored ? 'scored' : 'unscored'}
              type="submit"
              onClick={this.fetchUnscored}
            >
              UNSCORED
            </button>
            <button
              className={this.state.showUnscored ? 'unscored' : 'scored'}
              type="submit"
              onClick={this.fetchScored}
            >
              {' '}
              SCORED{' '}
            </button>
